...
# mydf = pd.DataFrame(np.array(([1,2,3], [4,5,6])),index=['mouse', 'rabbit'],columns=['one', 'two', 'three'])
# df.filter(items=['one', 'three'])

# mydf.filter(like='bbi', axis=0)
import json
input = ['sleeplessness',
              'neologism',
              'homicidal thoughts',
...
mydict = {'hypertensive disease': ['pain chest',
              'shortness of breath',
              'dizziness',
              'asthenia',
              'fall',
...
dictresult = {'hypertensive disease': 3363.0,
 'diabetes': 1421.0,
...
array = []
total = 0
for val in dictresult:
	total = int(dictresult[val]) + total

print (val)

for disease in mydict:
	print(disease)
	for x in mydict[disease]:
		print (x)
...
for dies in dictresult:
	finaldict['disease'] = dies
	finaldict['percentage'] = dictresult[dies] / total * 100
	# finaldict = mydict['dies']
	for i in mydict[dies]:
		for j in input:
			if i == j:
				symtomhash[i] = 'true'
			else:
				symtomhash[i] = 'false'
			#errase symptom hash
	finaldict['symptoms'] = symtomhash
	symtomhash = {}
	
	array.append(finaldict)
	finaldict = {}
	
	# finaldict[dies] = dictresult[dies]
# print (array)
js = json.dumps(array)
